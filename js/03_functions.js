...
console.log(addArrow(5, 7)); // Вызываем функцию, передаем числа 5 и 7.
...
displayCoordinates([5, -3]); // x=5, y=-3. 'z' возьмет значение по умолчанию 0.
...
console.log(sayHello("John")); // Вызываем sayHello, передаем имя "John". Получаем "Hello, John! Как дела?".
...
// Внимание: эта рекурсивная реализация Фибоначчи ОЧЕНЬ неэффективна для больших 'n', так как много раз пересчитывает одни и те же значения. Для больших n лучше использовать итеративный подход (цикл) или мемоизацию.
...
console.log("cube(3):", cube(3));   // Вызываем cube с основанием 3. Результат: 27.
...
setTimeout(calculator.add.bind(calculator), 100, 20); // Мы передаем в setTimeout НЕ сам метод, а его ПРИВЯЗАННУЮ копию (где this=calculator). setTimeout вызовет эту копию через 100мс, передав ей аргумент 20. Все сработает. Вывод (через 100мс): Значение: 35 (15 + 20).
...
function greetClb(name) { // Первая функция-колбэк.
  console.log(`Добро пожаловать, ${name}!`); // Просто выводит приветствие.
}
function notifyClb(name) { // Вторая функция-колбэк.
  console.log(`Уважаемый(ая) ${name}, ваша комната будет готова через 30 минут`); // Выводит уведомление.
}

// Функция высшего порядка, которая принимает колбэк:
function registerGuestClb(name, callback) { // Принимает имя 'name' и функцию 'callback'.
  console.log(`Регистрируем гостя ${name}!`); // Выполняет какое-то основное действие.
  callback(name); // Вызывает переданную ей колбэк-функцию, передавая ей 'name'. Основная функция решает, КОГДА вызвать колбэк.
}

// Вызываем registerGuestClb, передавая разные колбэки:
registerGuestClb("Манго", greetClb); // Передаем имя "Манго" и функцию greetClb как колбэк. registerGuestClb вызовет greetClb("Манго").
registerGuestClb("Поли", notifyClb); // Передаем имя "Поли" и функцию notifyClb как колбэк. registerGuestClb вызовет notifyClb("Поли").

// Еще пример с колбэками:
function deliverPizzaClb(pizzaName) { return `Доставляем пиццу ${pizzaName}.`; } // Колбэк 1: возвращает строку о доставке.
function makePizzaClb(pizzaName) { return `Пицца ${pizzaName} готовится, подождите...`; } // Колбэк 2: возвращает строку о готовке.

// Функция высшего порядка, принимающая колбэк для формирования сообщения:
function makeMessageClb(pizzaName, callback) { // Принимает название пиццы и колбэк.
  return callback(pizzaName); // Просто вызывает колбэк с названием пиццы и возвращает его результат.
}

// Вызываем makeMessageClb с разными колбэками:
console.log("Сообщение 1:", makeMessageClb("Роял Гранд", makePizzaClb)); // Передаем makePizzaClb как колбэк. Результат: "Пицца Роял Гранд готовится, подождите...".
console.log("Сообщение 2:", makeMessageClb("Ультрасыр", deliverPizzaClb)); // Передаем deliverPizzaClb как колбэк. Результат: "Доставляем пиццу Ультрасыр.".

// === Инлайн-колбеки (Callbacks) ===
// Подсекция о том, что колбэк-функцию можно определять прямо внутри вызова основной функции (инлайн).
...
// Вызов registerGuestInline с АНОНИМНЫМ инлайн-колбэком:
registerGuestInline("Манго", function (guestName) { // Вместо имени функции передаем 'function(...) {...}' прямо здесь.
  console.log(`Добро пожаловать инлайн, ${guestName}!`); // Тело колбэка.
}); // Конец вызова registerGuestInline.

// Вызов registerGuestInline с ИМЕНОВАННЫМ инлайн-колбэком (имя 'notifyInline' полезно для отладки, но не обязательно):
registerGuestInline("Поли", function notifyInline(guestName) { // Можно дать имя инлайн-функции.
  console.log(`Комната готова инлайн через 30 мин, ${guestName}`); // Тело колбэка.
}); // Конец вызова.

// Еще пример с инлайн-колбэком для пиццы:
...
// Вызов с инлайн-колбэком (анонимным):
makePizzaInlineTask("Ультрасыр", function (nameOfPizza) { // Определяем колбэк прямо тут.
  console.log(`Едим пиццу инлайн: ${nameOfPizza}`); // Тело колбэка.
});
...
numbersForEach.forEach(function (number, index) { // Передаем анонимную инлайн-функцию как колбэк.
  // forEach передает в колбэк три аргумента (мы используем первые два):
  // 1. 'number' - текущий обрабатываемый элемент массива.
  // 2. 'index' - индекс текущего элемента.
  // 3. (не используется здесь) - сам массив, по которому идет перебор.
  console.log(`  Индекс ${index}, значение ${number}`); // Выводим индекс и значение внутри колбэка.
}); // Конец вызова forEach.

// Пример использования forEach для подсчета суммы элементов массива:
function calculateTotalPriceForEach(orderedItems) { // Функция принимает массив чисел.
  ...
  orderedItems.forEach(function (item) { // Колбэк принимает текущий элемент 'item'. Индекс и сам массив нам здесь не нужны.
    totalPrice += item; // Добавляем текущий элемент к общей сумме.
  });
  return totalPrice; // Возвращаем итоговую сумму.
}
// Примеры вызова:
...
// Пример использования forEach для фильтрации массива (хотя для этого лучше подходит метод `filter`):
function filterArrayForEach(numbers, value) { // Принимает массив 'numbers' и пороговое значение 'value'.
  ...
  numbers.forEach(function (number) { // Колбэк получает текущее число 'number'.
    if (number > value) { // Проверяем условие фильтрации.
      filteredNumbers.push(number); // Если условие выполняется, добавляем число в новый массив.
    }
  });
  return filteredNumbers; // Возвращаем массив с отфильтрованными числами.
}
// Примеры вызова:
console.log("Фильтр > 3 из [1, 2, 3, 4, 5]:", filterArrayForEach([1, 2, 3, 4, 5], 3)); // Оставит [4, 5].
console.log("Фильтр > 38 из [12, 24, 8, 41, 76]:", filterArrayForEach([12, 24, 8, 41, 76], 38)); // Оставит [41, 76].

// Конец основной части примеров по функциям.
console.log("\n===== КОНЕЦ РАЗДЕЛА ФУНКЦИИ (ПОКА ЧТО) =====");

// --- Логика для выполнения примеров на странице functions.html ---
// Этот блок кода отвечает за работу кнопок "Запустить пример" на HTML-странице. Он использует DOM API для взаимодействия с HTML-элементами.

// Находим ВСЕ HTML-элементы с классом 'run-code' (это наши кнопки "Запустить пример"). Возвращается NodeList (похож на массив).
document.querySelectorAll('.run-code').forEach(button => { // Используем forEach для перебора каждой найденной кнопки.
  // Для КАЖДОЙ кнопки 'button' добавляем обработчик события 'click'.
  button.addEventListener('click', () => { // Стрелочная функция будет выполняться каждый раз при клике на кнопку.
    // Ищем элемент <pre><code>...</code>, который находится ВНУТРИ родительского элемента кнопки (<div class="code-block">).
    // button.parentElement - это <div class="code-block">
    // .querySelector('pre code') - ищет первый вложенный элемент 'code' внутри 'pre'.
    const codeElement = button.parentElement.querySelector('pre code');
    // Ищем элемент <div class="output"> ВНУТРИ того же родительского элемента. В него будем выводить результат.
    const outputDiv = button.parentElement.querySelector('.output');

    // Проверка: если не нашли элемент с кодом или элемент для вывода, выводим предупреждение в консоль и ничего не делаем.
    if (!codeElement || !outputDiv) {
      console.warn("Не найдены элементы 'pre code' или 'div.output' для кнопки 'Запустить'."); // Предупреждение в консоли браузера.
      return; // Прекращаем выполнение обработчика клика.
    }

    // Получаем значение атрибута 'data-example' у кнопки (если он есть, для возможной специфической логики). Здесь он не используется, но мог бы.
    const exampleId = button.getAttribute('data-example');
    // Очищаем содержимое div'а для вывода от предыдущих результатов.
    outputDiv.innerHTML = ''; // Устанавливаем HTML-содержимое в пустую строку.

    // --- Перехват console.log ---
    // Чтобы выводить результаты выполнения кода из примеров не в консоль браузера, а в наш <div class="output">,
    // мы временно "подменяем" стандартную функцию console.log.

    // Сохраняем оригинальную функцию console.log в переменную, чтобы потом ее вернуть.
    const originalConsoleLog = console.log;
    // Создаем массив, куда будем собирать все строки, которые должны были вывестись в консоль.
    const logs = [];

    // Переопределяем console.log: теперь при ее вызове будет выполняться НАША функция.
    console.log = (...args) => { // Используем Rest-параметр '...args', чтобы собрать все аргументы, переданные в console.log.
      // Форматируем аргументы для красивого вывода (объекты превращаем в JSON-строки).
      const formattedArgs = args.map(arg => { // map создает новый массив, применяя функцию к каждому элементу 'args'.
        if (typeof arg === 'object' && arg !== null) { // Если аргумент - объект (и не null),
          try { // Пытаемся превратить его в JSON-строку с отступами (null, 2).
            return JSON.stringify(arg, null, 2);
          } catch (e) { return '[Circular Object]'; } // Если объект циклический (ссылается сам на себя), JSON.stringify выдаст ошибку, ловим ее.
        }
        return String(arg); // Если не объект, просто превращаем в строку.
      });
      // Добавляем отформатированную строку в наш массив логов. join(' ') объединяет элементы массива в одну строку через пробел.
      logs.push(formattedArgs.join(' '));
      // ВАЖНО: Вызываем ОРИГИНАЛЬНЫЙ console.log, чтобы вывод все равно появлялся и в консоли браузера для отладки.
      originalConsoleLog.apply(console, args); // apply нужен, чтобы передать оригинальный контекст 'console' и аргументы 'args' как массив.
    };
    // --- Конец перехвата console.log ---

    // --- Выполнение кода примера ---
    try { // Используем блок try...catch для отлова возможных ошибок во время выполнения кода примера.
      // Получаем текстовое содержимое элемента <code> (это и есть код примера).
      const codeText = codeElement.textContent;
      // Создаем НОВУЮ функцию "на лету" из строки с кодом. Это ОПАСНО, если код может прийти из недоверенного источника (пользовательский ввод), но здесь код наш.
      // new Function(codeText) создает функцию, тело которой - это codeText.
      const userCode = new Function(codeText);
      // Выполняем созданную функцию. Весь код из <code> будет выполнен.
      userCode();

      // --- Вывод результата в <div class="output"> ---
      // Проверяем, были ли какие-то логи (вызовы перехваченного console.log).
      if (logs.length > 0) {
        // Если логи есть, создаем для каждой строки лога свой HTML-элемент <p> и добавляем его в outputDiv.
        logs.forEach(log => { // Перебираем массив логов.
          const p = document.createElement('p'); // Создаем новый элемент параграфа <p>.
          p.textContent = log; // Устанавливаем текст параграфа равным строке лога.
          outputDiv.appendChild(p); // Добавляем созданный <p> внутрь <div class="output">.
        });
      } else {
        // Если логов не было (код выполнился без console.log), выводим сообщение об этом.
        const p = document.createElement('p'); // Создаем <p>.
        p.textContent = "Код выполнен без вывода в console.log."; // Текст сообщения.
        p.style.fontStyle = "italic"; // Делаем текст курсивным.
        outputDiv.appendChild(p); // Добавляем в <div class="output">.
      }
    } catch (error) { // Если в блоке 'try' произошла ошибка,
      // Выводим сообщение об ошибке в <div class="output">.
      const p = document.createElement('p'); // Создаем <p>.
      p.textContent = `Ошибка выполнения: ${error.message}`; // Показываем сообщение об ошибке.
      p.style.color = '#ff5555'; // Делаем текст красным.
      outputDiv.appendChild(p); // Добавляем в <div class="output">.
    } finally { // Блок 'finally' выполняется ВСЕГДА после try или catch.
      // ВОЗВРАЩАЕМ оригинальную функцию console.log на место, чтобы не мешать остальному коду страницы.
      console.log = originalConsoleLog;
    }
    // --- Конец выполнения кода примера ---
  }); // Конец обработчика addEventListener.
}); // Конец forEach для кнопок.

// --- Конец файла 03_functions.js ---
// Просто комментарий, обозначающий конец файла.